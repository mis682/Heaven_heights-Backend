...
exports.getByDate = async (req, res) => {
  ...
  const report = await RegalGardenClubDailyReport.findOne({ formNumber: Number(formNumber), reportDate: date });
...
exports.saveDraft = async (req, res) => {
  ...
  const report = await RegalGardenClubDailyReport.findOneAndUpdate(
...
exports.submitReport = async (req, res) => {
  const report = await RegalGardenClubDailyReport.findById(req.params.id);
...
exports.unlockReport = async (req, res) => {
  const report = await RegalGardenClubDailyReport.findByIdAndUpdate(
    req.params.id,
...
exports.exportExcel = async (req, res) => {
  ...
  const sheet = workbook.addWorksheet(`RegalGardenClub_${report.formNumber}_${report.reportDate}`);
  sheet.columns = [
  ...
  res.setHeader("Content-Disposition", `attachment; filename=regal-garden-club-${form?.label || report.formNumber}-${report.reportDate}.xlsx`);
