...
const { getFormByNumber } = require("../constants/regalGardenClubForms");
...
exports.createSubmission = async (req, res) => {
  ...
  const submission = await RegalGardenClubSubmission.create({
  ...
  const messageLines = [
    `🏆 *${submittedBy}* submitted *${form.label}* (Regal Garden Club) — ${photos.length} checkpoints`,
    "",
...
exports.listSubmissions = async (req, res) => {
  ...
  const submissions = await RegalGardenClubSubmission.find(filter).sort({ submittedAt: -1 });
  res.json(
    submissions.map((s) => ({
      _id: s._id,
...
exports.getSubmission = async (req, res) => {
  const submission = await RegalGardenClubSubmission.findById(req.params.id);
