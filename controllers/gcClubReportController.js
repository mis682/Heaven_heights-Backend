...
const GCClubDailyReport = require("../models/GCClubDailyReport");
const { GC_CLUB_STATUS_OPTIONS } = require("../constants/gcClubReportStatus");
const { GC_CLUB_FORMS, getFormByNumber } = require("../constants/gcClubForms");
const { buildGCClubReportPdf } = require("../utils/gcClubReportPdf");
const { recordMetric } = require("../utils/requestMetrics"); // TEMP diagnostic

exports.meta = async (req, res) => {
  res.json({
    statusOptions: GC_CLUB_STATUS_OPTIONS,
    forms: GC_CLUB_FORMS.map((f) => ({
...
exports.getByDate = async (req, res) => {
  ...
  const report = await GCClubDailyReport.findOne({ formNumber: Number(formNumber), reportDate: date });
...
exports.saveDraft = async (req, res) => {
  ...
  const report = await GCClubDailyReport.findOneAndUpdate(
...
exports.submitReport = async (req, res) => {
  const report = await GCClubDailyReport.findById(req.params.id);
...
exports.unlockReport = async (req, res) => {
  const report = await GCClubDailyReport.findByIdAndUpdate(
    req.params.id,
...
exports.listSubmitted = async (req, res) => {
  ...
  const reports = await GCClubDailyReport.find(filter).sort({ reportDate: -1 });
...
exports.exportExcel = async (req, res) => {
  ...
  const sheet = workbook.addWorksheet(`GCClub_${report.formNumber}_${report.reportDate}`);
  sheet.columns = [
  ...
  res.setHeader("Content-Disposition", `attachment; filename=gc-club-${form?.label || report.formNumber}-${report.reportDate}.xlsx`);
...
exports.exportPdf = async (req, res) => {
  ...
  res.setHeader("Content-Disposition", `attachment; filename=gc-club-${form?.label || report.formNumber}-${report.reportDate}.pdf`);

  const __t0 = process.hrtime.bigint(); // TEMP diagnostic
  const doc = buildGCClubReportPdf(report, form);
