...
const { getFormByNumber } = require("../constants/gcClubForms");
...
exports.createSubmission = async (req, res) => {
  ...
  const submission = await GCClubSubmission.create({
...
exports.listSubmissions = async (req, res) => {
  ...
  const submissions = await GCClubSubmission.find(filter).sort({ submittedAt: -1 });
  res.json(
    submissions.map((s) => ({
      _id: s._id,
...
exports.getSubmission = async (req, res) => {
  const submission = await GCClubSubmission.findById(req.params.id);
